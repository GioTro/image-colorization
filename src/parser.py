...
parser = argparse.ArgumentParser(description="Specify parameters from cmndline.")
...
parser.add_argument(
    "--cpu", action="store_true", help="Override default. Default is gpu if available."
)

parser.add_argument(
    "--pretrained", action="store_true", help="Use the pretrained model."
)
...
parser.add_argument("--mkjson", action="store_true", help="make json param file")
    ...
    if OPT.load is not None:
        ...
        if not fname.exists():
            raise FileNotFoundError(
                f"{fname} not found use option --mkjson to dump default settings"
            )
